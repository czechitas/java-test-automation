...
Called on `asserter.applicationDetailsSection`:

| Method | Description |
|---|---|
| `asserter.applicationDetailsSection.checkPaymentMethod("Bankovní převod")` | Ensure the application is paid via bank transfer |
| `asserter.applicationDetailsSection.checkFirstName("Jan")` | Check that the student first name is _Jan_ |
...
- `asserter.applicationDetailsSection.checkTerm("05.02. - 09.02.2024")` - check that the term is _05.02. - 09.02.2024_
- `asserter.applicationDetailsSection.checkMessageContainsStudentLastName("Novak")` - ensure that _Zpráva pro příjemce_ contains student last name _Novak_
